...
def draw_card(text, out_path=None, dims=(9, 7), std_xy=1.0, text_size=20, bg_path=BG_PATH, max_angle=10, dy=0.45):
    ...
    # random parameters
    scale = np.random.rand() + 1  # 'zoom' level
    x0 = std_xy * np.random.randn()  # center of card
    ...
    rect = patches.FancyBboxPatch((x0 - w/2,y0 - h/2), w/1, h/1, linewidth=0, facecolor=card_color,
    ...
    for i, line in enumerate(lines):
        ...
        if len(lines) == 1:
            ...
            # left-align multi-line prompts, as in the original card set
            x = x0 - w / 2.8
            ha = 'left'
        print(f'text: x={x:.3f}, y={y:.3f}')
        if os.path.exists(FONT_PATH):
            # if we're on Heroku, use its closest approximation to Helvetica
            ax.text(x, y, line, color=text_color, fontproperties=fm.FontProperties(fname=FONT_PATH, size=text_size),
                    ha=ha, rotation=rand_angle, rotation_mode='anchor', transform=transform)
        else:
            ax.text(x, y, line, color=text_color, name='helvetica', size=text_size, 
                    ha=ha, rotation=rand_angle, rotation_mode='anchor', transform=transform)
